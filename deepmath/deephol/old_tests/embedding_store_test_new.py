...
# self.test_subdirectory = tempfile.mkdtemp(dir=FLAGS['test_tmpdir'])
# store = embedding_store.TheoremEmbeddingStore(MOCK_PREDICTOR)
# thm_db = proof_assistant_pb2.TheoremDatabase()
# for i in range(8):
#   thm = thm_db.theorems.add()
#   thm.conclusion = 'th%d' % i
# thms = _process_thms(thm_db.theorems)
# assertLen(thms, 8)

# print (thms)
# print (store.predictor)
...
class EmbeddingStoreTest:
  ...
  def setUp(self):
    ...
    assert len(self.thms) ==  8
#
  def test_init_embedding_store(self):
    assert self.store.predictor == MOCK_PREDICTOR
    self.assertIsNone()
    assert self.store.thm_embeddings == None
#
  def test_compute_embeddings_for_thms_from_db(self):
    ...
    assert store.thm_embeddings == np.allclose(np.array([[thm.__hash__(), 1] for thm in self.thms]))
#
...
if __name__ == '__main__':
  test = EmbeddingStoreTest
  test.test_init_embedding_store()
#   tf.test.main()
