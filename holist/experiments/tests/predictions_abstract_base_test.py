...
from holist.experiments import predictions
...
class PredictionsAbstractBaseTest(
    ...
    def _testSameEmbeddings(self, formulas):
        ...
        self.assertEqual(
            len(tac_pred[0]), NUM_TACTICS,
            'A probability should be predicted for each tactic (%d).' % NUM_TACTICS)
        self.assertEqual(
            len(tac_pred[1]), NUM_TACTICS,
            'A probability should be predicted for each tactic (%d).' % NUM_TACTICS)

        self.assertListEqual(
            tac_pred.tolist(),
            predictor.batch_tactic_scores(goal_embs).tolist(),
    ...
    def testDifferentEmbedding(self):
        ...
        self.assertEqual(
            len(set([tuple(emb) for emb in goal_embs])), len(self.goal_formulas),
            'Unique goals should be uniquely embedded.')
        self.assertEqual(
            len(set([tuple(emb) for emb in thm_embs])), len(self.formulas),
    ...
    def _assert_all_close_batch(self, batch1, batch2, message):
        for v1, v2 in zip(batch1, batch2):
            self.assertListEqual(v1.tolist(), v2.tolist(), msg=message)
    ...
    def testProofStateEmbedding(self):
        ...
        self.assertListEqual(expected_state_emb[1:], actual_state_emb[1:])
    ...
    def testProofStateEncoding(self):
        ...
        self.assertListEqual(goal_emb, actual_state_enc)
